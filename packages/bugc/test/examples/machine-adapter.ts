...
/**
 * Create a null Machine.State for evaluating pure expressions.
 *
 * This is used when evaluating slot expressions that don't need
 * access to actual machine state (e.g., $keccak256, $sum).
 * All accessors throw errors if called.
 */
export function createNullMachineState(): Machine.State {
  const notSupported = (name: string) => () => {
    throw new Error(`Machine state access not supported: ${name}`);
  };
  ...
  return {
    ...
    stack: {
      length: Promise.resolve(0n),
      peek: notSupported("stack.peek"),
    },

    memory: {
      length: Promise.resolve(0n),
      read: notSupported("memory.read"),
    },

    storage: {
      read: notSupported("storage.read"),
    },

    calldata: {
      length: Promise.resolve(0n),
      read: notSupported("calldata.read"),
    },

    returndata: {
      length: Promise.resolve(0n),
      read: notSupported("returndata.read"),
    },

    code: {
      length: Promise.resolve(0n),
      read: notSupported("code.read"),
    },

    transient: {
      read: notSupported("transient.read"),
    },
  };
}
