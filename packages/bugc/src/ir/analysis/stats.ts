...
export class IrStats {
  analyze(module: IrModule): IrStatistics {
  ...
  private countTemporaries(func: IrFunction): number {
  ...
  private collectTempsFromValue(inst: IrInstruction, temps: Set<string>): void {
    const valueFields = [
      "value",
      "left",
      "right",
    ...
    for (const field of valueFields) {
      if (field in inst) {
        const fieldValue = (inst as unknown as Record<string, unknown>)[field];
        if (fieldValue) {
  ...
  private countCfgEdges(func: IrFunction): number {
    let edges = 0;
  ...
  private findCriticalPaths(func: IrFunction): PathInfo[] {
